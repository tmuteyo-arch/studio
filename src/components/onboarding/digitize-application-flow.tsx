  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
          ...
          toast({ variant: 'destructive', title: 'Processing Error', description: 'Could not process the file.' });
      } finally {
  ...
  const onSubmit = async () => {
    ...
    try {
        ...
        const newApplication = {
          ...
          history: [
            { action: 'Locked', user: user.name, timestamp: new Date().toISOString(), notes: `Digitized paper record archived directly. Account Type: ${clientType}` },
          ],
          comments: [],
        } as Application;
    ...
    <div className="p-4 sm:p-8 max-w-5xl mx-auto">
      ...
      <Dialog open={isScanning} onOpenChange={(isOpen) => !isOpen && stopScan()}>
        <DialogContent className="max-w-xl bg-background border-primary/20">
          <DialogHeader><DialogTitle>Scan Page: {documentType}</DialogTitle></DialogHeader>
        ...
        <DialogContent className="max-w-4xl h-[80vh] flex flex-col rounded-2xl overflow-hidden border-none p-0">
            ...
            <div className="flex-1 bg-black relative flex items-center justify-center min-h-0">
                {previewPage?.type === 'image' ? (
                    ...
                    <iframe src={previewPage?.dataUri} className="w-full h-full border-none" title="PDF Preview" />
                )}
            </div>
        </DialogContent>
      </Dialog>
